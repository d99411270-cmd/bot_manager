...
На 2026-08-26 (локальный тестовый каталог, конкуренты без навязывания, strict multi-intent orchestration, безопасная invalid-phone формулировка и unit-price recovery):

- `uv run --with pytest --with pytest-asyncio --with httpx pytest -q` — проверка после изменений выполняется локально.
- `uv run --with ruff ruff check .` — проверка после изменений выполняется локально.
- `uv run --with ruff ruff format --check .` — проверка после изменений выполняется локально.
- `git diff --check` — проверка после изменений выполняется локально.
