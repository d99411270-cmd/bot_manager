...
- Продукт и объём в CRM по-прежнему не пишутся без телефона. Для каталогового диалога без контакта отдельно сохраняется подтверждённый `current_interest`: если ответ DeepSeek явно отражает категории/подкатегории из найденных позиций, follow-up использует этот интерес вместо поиска по фразе «А какие цены?». При наличии контакта этот интерес также синхронизируется в `product`. При пустом каталоге явно названный товар не записывается как подтверждённый: исходный запрос хранится отдельно в `catalog_no_match_query`, статус остаётся `уточнение продукта`, а follow-up повторяет честный no-match без объёма/менеджерского обещания.
...
- Обязательные столбцы листа `Клиенты` не расширялись. `deal_stage` в таблицу не пишется. `catalog_no_match_query` сохраняется в совместимом `комментарии` как `Товар не найден: ...` и не превращается в `интересующая продукция`.
