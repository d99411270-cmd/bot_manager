...
- На 2026-08-26 (TDD: в Telegram прайс по «можно/есть» сразу файлом, почта только по явной просьбе, без push/deploy):
- `PYTHONPATH=src .venv/bin/pytest -q` — `475 passed`.
- `ruff check .` / `ruff format --check .` / `git diff --check` — чисто.
- Коммит локальный: `Send price list in Telegram by default`. Пуш и деплой **не** выполнялись.
