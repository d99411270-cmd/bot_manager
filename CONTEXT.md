...
- `generated_price_list()` в `product_catalog.py` — источник истины для клиентского прайса: ровно 30 primary, товар/SKU/производитель/фасовка/цена, без дат, наличия и точных остатков. Pending-action прайса (`price_list_requested`) отделён от email-слота. В Telegram обычный `прайс`/`каталог`/`прайс есть?`/`прайс можно?` сразу отдаёт attachment в чат и **не** спрашивает почту. Полный ассортимент (`wants_full_assortment`: «вся», «все категории», «все продукты», «весь ассортимент») — сначала почта для прайса, ответ на email «Ок, почту записал. Вам отправят актуальный прайс.» (mailer нет, не «я отправил»); отказ «в чат / сюда / не на почту» отдаёт файл. Почта по явному «на почту / на email» тоже сохраняется с этим ack, без `PRODUCT_QUESTION`. После вопроса почты «в чат / сюда / не на почту / на почту не надо» восстанавливают pending и отдают файл, не cancel. «пишите в этот чат» без предшествующего прайса — `contact_skipped`, не файл. Отказ (`прайс не надо`, `прайс повторно не надо`, `файл уже прислали`) отменяет pending, не стартует новый запрос. `mark_price_list_sent` только после реального `answer_document` / QA ACK; `handle()` `sent_at` не ставит. Автооффер «Могу проконсультировать… выслать актуальный прайс» **никогда** не префиксится, в том числе на «какие овощи?». После отправки повторного оффера тоже нет.
...
- Для прайса не добавлялся email-транспорт: почта — слот контакта и лид, не mailer. В Telegram явный `прайс есть?` / `прайс можно?` / `давайте прайс` сразу файлом в чат. Полный ассортимент («вся» / «все категории» / «все продукты которые есть») — сначала спросить почту, на email сказать «вам отправят», не врать «я отправил»; отказ от почты / «в чат» — файл. Почту по явной просьбе «на почту / на email» тоже принимаем без фейковой отправки. `answer_document` обёрнут в безопасную обработку ошибки; успех и `sent_at` только после реальной отправки. Postgres, пуш и деплой не делались.
...
- На 2026-08-27 (TDD: live 679025492 — «вся/все категории» = полный ассортимент, прайс на почту сначала, файл при отказе; без push/deploy):
- `PYTHONPATH=src .venv/bin/pytest -q` — `557 passed`.
