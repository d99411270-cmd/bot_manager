...
- `product_catalog.py` ищет markdown в `catalog/` (env `STOKOZAVR_CATALOG_DIR`, `catalog/` в корне репо, пакетные данные в wheel). `search("фрукты")` находит `frukty.md`. Пустой поиск — список категорий из заголовков.
...
- `respond` отдаёт OpenAI-compatible `tools` с `search_catalog(query)`. Для сообщений с известным продуктом сервис передаёт локальный каталог в `respond_with_catalog`, поэтому DeepSeek сам понимает сомнение «зависит от цены» по истории/профилю и не повторяет известный объём. Цены и статусы в свободном ответе проверяются against `catalog_result` этого вызова; пустой каталог не позволяет заявлять цену/наличие. Одноразовый `repair_response` получает профиль, историю, исходный вопрос, причину отбраковки и каталог; рекурсия запрещена. Intake tools не получает.
...
- `uv run --with pytest --with pytest-asyncio --with httpx pytest -q` — `216 passed in 9.54s`.
