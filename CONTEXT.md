...
- При неизвестной информации, пустом каталоге, ошибке DeepSeek, опасном утверждении, вопросе про товар/цену без валидного AI-ответа: основной ответ → repair → максимум один open-dialog recovery (всего не более 4 AI-вызовов с повторным recovery на подтверждённом каталоге). Open-dialog получает профиль, релевантную историю, причину отказа и каталог, может выбрать следующий ход и один вопрос, но коммерческие факты берёт только из memory/catalog. Если AI не дал безопасный ответ — сохраняются `pending_manager_question`/`needs_human` и используется grounded deterministic fallback без фиктивного обещания вернуться.
...
- `uv run --with pytest --with pytest-asyncio --with httpx pytest -q` — `238 passed` после изменений.
