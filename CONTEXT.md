...
На 2026-08-26 (локальный тестовый каталог, конкуренты без навязывания, strict multi-intent orchestration, безопасная invalid-phone формулировка, generic fallback rejection, open-dialog recovery и unit-price за банку):

- `uv run --with pytest --with pytest-asyncio --with httpx pytest -q` — `242 passed` после изменений.
