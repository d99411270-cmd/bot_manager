...
- `respond` отдаёт OpenAI-compatible `tools` с `search_catalog(query)`. Для каталоговых вопросов сервис заранее выполняет локальный поиск и передаёт его в `respond_with_catalog`; модель всё равно может вызвать tool. Цикл: tool_call → поиск → tool result → финальный JSON `{reply, product, volume, needs_human}`. Одноразовый `repair_response` получает исходный вопрос, причину отбраковки и каталог; рекурсия запрещена. Intake tools не получает.
...
- `uv run --with pytest --with pytest-asyncio --with httpx pytest -q` — `218 passed in 9.96s`.
- `uv run --with ruff ruff check .` — `All checks passed!`.
- Новые сценарии: объёмы «36 банок»/«20 упаковок»/«полпаллеты»/«50 литров», бюджет «на 10000 рублей» с расчётом `11 упаковок + 210 ₽`, разнообразные соседние вопросы и полное подавление конкурентных записей; причины `exception`, `needs_human`, `unsafe_reply`/`invalid_reply` логируются без текста ответа; repair вызывается не более одного раза, получает исходный вопрос/причину/каталог и не может протащить цену из пустого каталога.
- `uv run --with ruff ruff check .` — `All checks passed!`.
