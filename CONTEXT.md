...
- После квалификации `_handle_ai` применяет `turn.product` / `turn.volume`. Подтверждённый объём не перезаписывается фрагментом фасовки (`12×340г`) и не затирается unit-price «за литр/кг»; фасовка не становится первым объёмом. Явные `36 банок` / `20 кг` / `10 упаковок` / `4 сетки` / `200 литров` / `Литров 200` сохраняются. `parse_requested_quantity` читает словесные числительные (`два/две короба`), обратный порядок «литров 200» и не берёт размер фасовки из «короб 10 кг 820». Нецелое количество даёт `NearestPackQuote` (floor+ceil), не выдуманную смесь: 20 кг яблок → 2 короба = 1640 ₽; 100 кг картофеля → 4 сетки = 3000 ₽; 200 л морса → 33/34 упаковки. Расчёт кладётся в `catalog_result` как tool result для DeepSeek, не как готовая заглушка вместо модели. AI-ответ «какой объём?» при известном volume отбраковывается.
...
- Локальный тестовый каталог: 30 разных основных товарных позиций и 30 конкурентных записей в `catalog/*.md`, по одному конкуренту на каждый основной SKU. Обычный поиск и прайс используют только primary. На конкретной primary-позиции (есть? / почём / наличие SKU) Иван **сам один раз** называет linked-конкурента дороже, не дожидаясь «сравните». Не в категорийном списке и не в прайс-файле. Opt-in «подешевле»/«есть варианты»/«сравнить» или недоступность основного товара по-прежнему открывает сравнение. В диалоге счётчик считает **видимые** упоминания (бренд/SKU конкурента или его цена с ₽, дамп каталога), максимум два за разговор, не подряд. «Сравните» может быть вторым разом. Слова «сравн/альтернатив/конкурент» в ответе больше не сжигают весь текст; третье сравнение снимает конкурента и оставляет primary, без «уточню и вернусь». «вариант» в regex нет. «подешевле» может назвать 890 без 990 — счётчик не растёт, пока конкурент не назван.
- `product_catalog.py` принимает коммерческую запись только при наличии всех полей: категория → подкатегория → SKU → производитель → фасовка → цена → статус наличия → дата обновления ISO; неполные строки игнорируются.
...
- `generated_price_list()` в `product_catalog.py` — источник истины для клиентского прайса: ровно 30 primary, товар/SKU/производитель/фасовка/цена, без дат, наличия и точных остатков. Pending-action прайса (`price_list_requested`) отделён от email-слота. В Telegram обычный `прайс`/`каталог`/`прайс есть?`/`прайс можно?` сразу отдаёт attachment в чат и **не** спрашивает почту. Почта только по явному «на почту / на email»: слот сохраняется с коротким подтверждением, без `PRODUCT_QUESTION` и без утверждения, что файл ушёл (mailer нет). После вопроса почты «в чат / сюда / не на почту / на почту не надо» восстанавливают pending и отдают файл, не cancel. «пишите в этот чат» без предшествующего прайса — `contact_skipped`, не файл. Отказ (`прайс не надо`, `прайс повторно не надо`, `файл уже прислали`) отменяет pending, не стартует новый запрос. `mark_price_list_sent` только после реального `answer_document` / QA ACK; `handle()` `sent_at` не ставит. Автооффер «Могу проконсультировать… выслать актуальный прайс» **никогда** не префиксится. После отправки повторного оффера тоже нет.
...
- Для прайса не добавлялся email-транспорт: почта — слот контакта, не доставка. В Telegram файл уходит в текущий чат по умолчанию (`прайс есть?` / `прайс можно?` / `давайте прайс`). Почту спрашиваем только по явной просьбе «на почту / на email»; не врём, что отправили. `answer_document` обёрнут в безопасную обработку ошибки; успех и `sent_at` только после реальной отправки. Postgres, пуш и деплой не делались.
...
- На 2026-08-26 (TDD: сам один раз назвать linked-конкурента дороже на primary-позиции; прайс в Telegram файлом):
- `PYTHONPATH=src .venv/bin/pytest -q` — после merge проверю на main.
...
Slice I: в Telegram «прайс есть/можно» сразу файлом, почта только по явной просьбе — закоммичено локально, **не** на Beget. Production `@Stokozavr_manager_bot` всё ещё `1b5169e`. После явного пуш/деплоя проверить живых Сергея/Наталью/Андрея: «прайс можно?» = файл в чат, не EMAIL_QUESTION; «на почту не надо, киньте в чат» = файл, не «без прайса». Осталось: живой amoCRM, name-frequency, handoff adapter в стенде.
