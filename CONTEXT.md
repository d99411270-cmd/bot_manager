...
- После квалификации `_handle_ai` применяет `turn.product` / `turn.volume`. Подтверждённый объём не перезаписывается фрагментом фасовки (`12×340г`) и не затирается unit-price «за литр/кг»; фасовка не становится первым объёмом. Явные `36 банок` / `20 кг` / `10 упаковок` / `4 сетки` / `200 литров` / `Литров 200` / `3 мешка` / `пару банок` сохраняются. `parse_requested_quantity` читает словесные числительные (`два/две короба`, `пару/пара банок`), обратный порядок «литров 200» и не берёт размер фасовки из «короб 10 кг 820». Нецелое количество даёт `NearestPackQuote` (floor+ceil), не выдуманную смесь: 20 кг яблок → 2 короба = 1640 ₽; 100 кг картофеля → 4 сетки = 3000 ₽; 200 л морса → 33/34 упаковки; 2 банки маринованных → от упаковки 6 шт / 860 ₽. Если SKU+volume известны и quote есть, пустой «устраивает?» / stub «не могу подтвердить» отбраковываются: `catalog_result` обязан содержать totals, клиенту уходит grounded quote. Расчёт кладётся в `catalog_result` как tool result для DeepSeek, не как готовая заглушка вместо модели. AI-ответ «какой объём?» при известном volume отбраковывается. Подтверждённый `requested_slot` — владение: смешанный вопрос про сумму не переспрашивает «во сколько?».
