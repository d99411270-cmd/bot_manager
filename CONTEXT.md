...
- Вопрос про ассортимент, фрукты или цену **не** подменяется шаблоном `PRODUCT_QUESTION` / `PRODUCT_ASSORTMENT`. Имя и телефон из такого сообщения сохраняются тихо. Если AI дал валидный `reply` — его и шлём. Fallback только `FALLBACK` «Я уточню этот вопрос и вернусь к вам.»
...
- Локальный тестовый каталог: 30 разных основных товарных позиций и 30 конкурентных записей в `catalog/*.md`, по одному конкуренту на каждый основной SKU. Конкурент совпадает по категории, подкатегории и фасовке, связан через `Для SKU` и всегда дороже. Обычный поиск показывает только основной товар; `подешевле`/`сравнить`/`варианты` добавляют только его конкурента. Реальных коммерческих данных в проекте нет.
- `product_catalog.py` принимает коммерческую запись только при наличии всех полей: категория → подкатегория → SKU → производитель → фасовка → цена → статус наличия → дата обновления ISO; неполные строки игнорируются.
- В записях каталога поддерживаются отдельные конкурентные варианты через `Тип: конкурент` и `Для SKU`. `search_catalog` возвращает их только для соответствующего товара и только при запросе «подешевле», «есть варианты», «сравнить» или при недоступности основного товара; обычный поиск конкурентов не показывает.
...
- Профиль хранит `original_interests` отдельно от `current_interest`, чтобы переход с творожков на напитки/сок не затирал исходный интерес; `needs_human` сохраняется в совместимом поле комментария CRM.
...
- `respond` отдаёт OpenAI-compatible `tools` с `search_catalog(query)`. Модель сама решает вызвать tool. Цикл: tool_call → поиск → tool result → финальный JSON `{reply, product, volume, needs_human}`. Без tool нельзя называть конкретные позиции. Intake tools не получает.
...
- При неизвестной информации, ошибке DeepSeek, опасном утверждении, вопросе про товар/цену без валидного AI-ответа: «Я уточню этот вопрос и вернусь к вам.»
...
- `.venv/bin/python -m pytest -q` — `190 passed in 10.20s`.
- `.venv/bin/ruff format --check .` — `53 files already formatted`.
- `.venv/bin/ruff check .` — `All checks passed!`.
- Новые сценарии: `search("фрукты")` → честное отсутствие подтверждённых позиций без 780/420; DeepSeek mock `tool_call search_catalog` затем reply из tool result; «какие фрукты есть?» без телефона не содержит `PRODUCT_QUESTION`; захват имени не перебивает ответ про товар; цены не выдумываются.
- `START_TEXT` точный. `request_contact` всегда `False`. Колонки Sheets не ломались.
- Пуш и деплой **не** выполнялись.

Ранее на 2026-08-25:

- `.venv/bin/python -m pytest -q` — `127 passed` после фазы 1 (оркестратор + context_builder + sales_state + v4-flash).
