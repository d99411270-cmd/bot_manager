...
- Каталоговые quote formatters: `Цена` в markdown уже содержит единицу (`410 ₽ за мешок`, `640 ₽ за упаковку`). `format_catalog_pack_price` не дописывает `за {packaging}` / `за упаковку` повторно: line-total моркови «410 ₽ за мешок 10 кг», unit морса «6 x 1 л, 640 ₽ за упаковку». Каталог не меняли.
- Live 679025492 after `7b4f2fc`: `димас` мапится на Дмитрий рядом с диман/дима; Саша не трогаем. Каталоговые fallback (`grounded_search_reply` / quote formatters) больше не префиксуют `{name}, `. `_finish` снимает ведущий вокатив, если предыдущий ход бота уже назвал имя (кроме «Очень приятно, {name}» и «Спасибо, {name}»). `_FULL_ASSORTMENT_CATALOG_STEMS` включает `продукц` — «вся продукция» / «В целом интересно вся продукция» это полный ассортимент, не dump категорий. Intake `vague_interest` (bool, default False, JSON `.get`) после вопроса «что интересует» («пока не знаю», «разное», «посмотрим») идёт в тот же `FULL_ASSORTMENT_EMAIL_OFFER`; не на слоте телефона/почты и не после «какие овощи?».
...
- На 2026-08-28 изолированный live QA R19 на Beget `/tmp/stokozavr-qa-round19` (код с правкой phone/consult/brand; production `/opt` и MAX **не** трогали, пуш/деплой **нет**). Oracle из JSONL-транскриптов `qa-dialogues/round-19/p1.json`…`p6.json`, не driver.log. Все 6 персон **pass**: Димас→Дмитрий и 41 ₽/кг; «вся продукция» / «пока не знаю» → прайс на почту «Вам отправят», без дампа категорий и без файла; манго — честный нет, огурцы маринованные 860 / 12 банок = 1720; гречка своя 730, сети 810 без чужого бренда, «Крупяной берег» не наш, после «в чате» телефон не переспрашивал; морс 106.67 ₽/л, не вода 33.33. Наблюдение R19 (закрыто локально, без пуша): шаблон расчёта дублил «за мешок за мешок» / «за упаковку за упаковку», потому что `Цена` в каталоге уже содержит «за …», а formatter писал `за {packaging}` ещё раз.
...
- На 2026-08-27 (TDD: live QA R14 roots — phone-slot catalog search; unit quote not gated on qualified; без push/deploy):
- `PYTHONPATH=src .venv/bin/pytest -q` — `631 passed`.
- `ruff check .` / `ruff format --check .` / `git diff --check` — чисто.
- Слот телефона/почты больше не отключает **поиск** по `semantic.product`, если клиент сам назвал товар («манго есть?» → честный no-match, не выдуманные сок/нектар). Запись product по-прежнему через `_may_write_commercial_facts`. Intake JSON без названия в тексте не становится no-match. Structured `unit_quote`/`line_quote` идут в `_handle_ai` и без `is_qualified`; ответ без `106.67 ₽/л` отбраковывается, recovery даёт unit price. Пуш и деплой **не** выполнялись. CRM не трогали.
...
R13 локально без commit/push/deploy: «прив я ванек» и слот имени при greeting пишут официальное имя (Иван/Света), частицы дык/ну/блин/короче/типа/ваще/вообще не блокируют «еда»/«продукты питания».
